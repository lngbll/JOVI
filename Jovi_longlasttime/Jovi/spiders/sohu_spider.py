...
class SohuSpiderSpider(scrapy.Spider):
    ...
    log_dir = os.path.join(os.path.split(__file__)[0], '..', '..', 'log', 'sohu_spider')
    ...
    def parse_sports_2(self, response):
        ...
        if articles:
            ...
            meta['pageNum'] += 1
            next_page = 'https://v2.sohu.com/integration-api/mix/region/{}?adapter=pc&page={}&pvId={}'.format(
                meta['channelId'], meta['pageNum'], meta['pvid'])
            yield scrapy.Request(url=next_page, meta=meta, callback=self.parse_sports_2)

    def parse_second_nav(self, response):
        meta = response.meta
        meta['pgNum'] = 1
        thirdTag_nodes = response.xpath(
            '//div[@class="second-nav"]/div[@ data-role="box"] |//div[@class="category-second-nav"]/div[contains(@class,"category-common") and position()>1]')
    ...
    def parse_json(self, response):
        ...
        if data:
            ...
            meta['pgNum'] += 1
            next_page = 'http://v2.sohu.com/public-api/feed?scene=CATEGORY&sceneId={}&page=2&size=20'.format(
                meta['sceneId'])
