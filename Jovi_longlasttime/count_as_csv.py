...
path = os.path.join(os.getcwd(),'数据统计.xlsx')

dir = os.path.join(os.getcwd(),'定期更新{}'.format(date))
zip_file = os.path.join(os.getcwd(),'定期更新{}.zip'.format(date))

book = load_workbook(filename=path)

web = ['IT之家', 'Zaker新闻', '东方头条', '今日头条', '凤凰网', '手机网易网', '手机新浪网', '搜狗微信', '搜狐网', '腾讯新闻',
       '微博长文', '微博短文', '新华网', '央视新闻', '一点资讯', '人民网']  # 共15个网站
...
if __name__ == '__main__':
    os.chdir('e:\\Jovi_longlasttime\\新闻内容')
    move_dir(dir)
    main()
    remove_csv(dir, date)
    zip(dir, zip_file)
